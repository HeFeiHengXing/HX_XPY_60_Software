...
Private Declare Function APIBeep _
                Lib "kernel32" _
...
Public file_write_lock   As Boolean                                                     ' File write lock for out put
...
'输入: 完整路径, 目录分隔符
'返回: 文件名(带扩展名)
Public Function GetPathFromFileName(ByVal strFullPath As String, _
...
Public Sub getFileWriteLock()
    
    Do While True
        
        If file_write_lock = False Then
...
Public Sub releaseFileWriteLock()
