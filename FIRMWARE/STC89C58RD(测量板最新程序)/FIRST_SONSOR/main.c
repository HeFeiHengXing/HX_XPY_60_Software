...
//====================
#define color_maxvalue 0x7ff //10位精度时
#define color_dark  0x10		 
...
//unsigned char Previous_state=0;//当前状态的前一个状态
unsigned char rect_flag=0;
...
{  
  ...
  {      
    //==================点灯====================================
    if(Light_COMMAND >= 0x01 && Light_COMMAND <= 0x14)		//
    { 
    ...
    if(COMMAND == 0xC1) //201检测颜色,所有板子同时检测
    { 
		COM_send_char(0xC1);
    ...
    //===============校准=======================================
    if((COMMAND>=0x3D) && (COMMAND<=0x50))	
    {
	  //BUFF_COMMAND = COMMAND;
      COMMAND = COMMAND - 0x3C;
	  rect_flag=1;
      GET_COLOR(COMMAND);
      COMMAND=0xff;
    }                   
...
void com_interrupt(void) interrupt 4 using 0  //串口中断,接收命令处理相关操作
{
  ...
  if((k == 0xc1) || ((k>=0x3D) && (k<=0x50)) || (k == 0xCA) )	//检测、查询、校准
  {
    COMMAND = k;
  }

  if((k >= 0x01 && k <= 0x14) || k == 0x80)			
  {
    Light_COMMAND = k;
  }
//  if((k>=0x3D) && (k<=0x50))
...
void GET_COLOR(unsigned char  sensor_index )
{
	unsigned int F[4];                //相对于白平衡获得的RGB值
    unsigned int M[4];                //相对于白平衡获得的RGB值
    unsigned int T[4];                //相对于白平衡获得的RGB值
	EA=0;
...
unsigned int Count(unsigned int BALANCE)
{
	...
	TCON=0x00;
		...
		TL0=LSB(65536-BALANCE);////定时器0定时BALLANCE这么长时间
		TH1=0;
		TL1=0;
		TR0=1;
		TR1=1;
