...
{  
  ...
  {      
    //==================点灯====================================
    if(Light_COMMAND >= 0x15 && Light_COMMAND <= 0x28)		//
    { 
	  Light_COMMAND -= 0x14;
...
//	  Light_COMMAND += 0x14; 
    ...
    if(COMMAND == 0xC2) //201检测颜色,所有板子同时检测
    { 
			COM_send_char(0xC2);
    ...
    //===============校准=======================================
    if((COMMAND>=0x51) && (COMMAND<=0x64))	//
    {
	//  BUFF_COMMAND = COMMAND;
      COMMAND = COMMAND - 0x50;
	  rect_flag=1;
      GET_COLOR(COMMAND);
      COMMAND=0xff;
    }                   
...
void com_interrupt(void) interrupt 4 using 0  //串口中断,接收命令处理相关操作
{
  ...
  if((k == 0xc2) || ((k>=0x51) && (k<=0x64)) || (k == 0xCB) )
  {
    COMMAND = k;
  }

  if((k >= 0x15 && k <= 0x28) || k == 0x80)
  {
    Light_COMMAND = k;
  }
//  if((k>=0x3D) && (k<=0x50))
...
/*----------------------------------------------------------*/
//****************************************************************************************
unsigned int Count(unsigned int BALANCE)//原  uint
{
	unsigned int temp,m;
	TMOD = 0x51;
	TCON=0x00;
